...
const unsigned int touchPins[numTouchPins] = {0,1,15,16,17,18,19,22,23}; 

bool serial = false; // Serial on/off
...
const unsigned int printInterval = 250;
	...
	for (unsigned int i = 0; i < numTouchPins; i++) {
		...
		if (smoothingTime[i] - pastSmoothingTime[i] >= smoothingInterval) {
			...
			if(average[i] != previous[i]) {
				...
				usbMIDI.sendControlChange(i+1, average[i], 1); 
			}
		}
	}
